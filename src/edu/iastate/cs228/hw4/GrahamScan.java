...
public class GrahamScan extends ConvexHull
{
	...
	public void constructHull()
	{
		...
		//find all vertices that make a "left turn"
	...
	public void setUpScan()
	{
		...
		//make sure that there are no points with same polar angle.

		int m = 1; // starting index of array
		for (int i=1; i<pointsNoDuplicate.length; i++)
		{
			// Keep removing points while angle of point and its neighbor is the same
			// with respect to lowest point
			PolarAngleComparator comparator = new PolarAngleComparator(lowestPoint,true);
			while (i < pointsNoDuplicate.length-1 && comparator.comparePolarAngle(pointsNoDuplicate[i],
					pointsNoDuplicate[i+1]) == 0) i++;


			pointsNoDuplicate[m] = pointsNoDuplicate[i];
			m++;  // Update index of array
		}
	}
    ...
    {
		//calculate cross product. Redundant, but easier than creating a new comparator object every time.
