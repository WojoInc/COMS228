...
public class JarvisMarch extends ConvexHull
{
	...
	public void constructHull()
	{
		...
		for (int i = hullVertices.length-1; i >=0 ; i--) {
			if(!rightChain.isEmpty()) hullVertices[i] = rightChain.pop();
			else hullVertices[i] = leftChain.pop();
		}
	...
	public Point nextVertex(Point v)
	{
		...
		Point lowestAngle = pointsNoDuplicate[0];
		for (int i = 0; i< pointsNoDuplicate.length; i++) {
			if(pointsNoDuplicate[i].equals(v));
			else if(comp.compare(lowestAngle, pointsNoDuplicate[i])>0||lowestAngle.equals(v)) lowestAngle = pointsNoDuplicate[i];
		}
		return lowestAngle;
	}
}
